...
from general.config import NUM_REF_NEIGHBOR, MAXIMUM_REFERENCE_ANGLE, TARGET_ANGLE, NEIGHBOR_NUM, RADIUS_NUM, RADIUS
from general.geometry import Vertex, Segment


def reference_state(boundary, reference_point, area_ratio, static):
    ...
    r_points = _radius_points(boundary, reference_point, area_ratio, static, base_length)
    return {'reference_point': reference_point, 'neighbors': neighbors,
            'base_length': base_length, 'state': r_points.flatten()}

def _radius_points(boundary, rp, area_ratio, static, base_length):
...
def reference_candidates(boundary):
