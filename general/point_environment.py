...
class PointEnvironment:
    ...
    def get_state(self):
        self.neighbors = self.boundary.get_neighbors(self.reference_point, num_points=self.neighbor_num)
        self.base_length = round(sum(self.neighbors[i].distance_to(self.neighbors[i - 1])
                                     for i in range(1, len(self.neighbors))) / self.neighbor_num, 4)
