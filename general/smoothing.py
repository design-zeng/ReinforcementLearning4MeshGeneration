    ...
    def find_side_vertex(self, vertex, _next_v, next_v, nn_v, v_angle):
        ...
        while True:
            ...
            if target_angle <= v_angle:
                failed = True
                break
            clockwise_boundary = self.clockwise_vertices(vertex, connected_vs)
            if self.is_inside_boundary(vertex, n_v, clockwise_boundary, _next_v, next_v):
                break
            else:
                target_angle -= 5
            ...
            if v_angle <= 90:
                ...
                while True:
                    ...
                    if target_angle >= 135:
                        failed = True
                        break
                    clockwise_boundary = self.clockwise_vertices(self.updated_boundary.vertices[index], connected_vs)
        ...
        if len(neighbors) or len(c_neighbors):
            ...
            while True:
                ...
                if times >= 10:
                    ...
                    break
                clockwise_boundary = self.clockwise_vertices(vertex, connected_vs)
                if self.is_inside_boundary(vertex, n_v, clockwise_boundary, left_v, right_v):
                    break
                else:
                    times += 1
            if not failed:
